...
class StripWidget(cmdui.AbstractCmdWidget):
    ...
    def __init__(self, repoagent, rev=None, parent=None, opts=None):
        # type: (RepoAgent, Optional[Text], Optional[QWidget], Optional[Dict[Text, Any]]) -> None
    ...
    def get_rev(self):
        ...
        if not revstr:
            return None
        try:
            rev = hglib.revsymbol(self.repo, revstr).rev()
    ...
    def runCommand(self):
        ...
        # TODO: use cmdutil.checkunfinished()+cmdutil.bailifchanged()?
...
def createStripDialog(repoagent, rev=None, parent=None, opts=None):
    # type: (RepoAgent, Optional[Text], Optional[QWidget], Optional[Dict[Text, Any]]) -> cmdui.CmdControlDialog
    if opts is None:
        opts = {}
