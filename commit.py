...
def readopts(repoagent):
    # type: (RepoAgent) -> CommitOpts
    opts = {
...
def mergecommitmessage(repo):
    # type: (localrepo.localrepository) -> Text
    wctx = repo[None]
...
def _getUserOptions(opts, optionlist):
    # type: (CommitOpts, Iterable[Text]) -> List[Text]
    out = []
...
def _mqNewRefreshCommand(repo, isnew, stwidget, pnwidget, message, opts, olist):
    # type: (localrepo.localrepository, bool, status.StatusWidget, QLineEdit, Text, CommitOpts, Iterable[Text]) -> Optional[CmdLines]
...
class CommitWidget(QWidget, qtlib.TaskWidget):
    ...
    def __init__(self, repoagent, pats, opts, parent=None, rev=None):
        # type: (RepoAgent, List[bytes], Dict[Text, bytes], Optional[QObject], Optional[Text]) -> None
    ...
    def commitSetupButton(self):
        # type: () -> QToolButton
        ispatch = lambda r: b'qtip' in r[b'.'].tags()  # type: Callable[[localrepo.localrepository], bool]
        notpatch = lambda r: b'qtip' not in r[b'.'].tags()  # type: Callable[[localrepo.localrepository], bool]
        def canamend(r):
    ...
    def getBranchCommandLine(self):
        # type: () -> Tuple[Optional[CmdLines], bool]
        '''
...
class DetailsDialog(QDialog):
    ...
    def __init__(self, repoagent, opts, userhistory, parent, mode='commit'):
        # type: (RepoAgent, CommitOpts, List[Text], QWidget, Text) -> None
