...
class TagDialog(QDialog):
    ...
    def __init__(self, repoagent, tag=b'', rev='tip', parent=None, opts=None):
        ...
        ctx = hglib.revsymbol(repo, hglib.fromunicode(rev))
        form.addRow(_('Revision:'), QLabel('%d (%s)' % (ctx.rev(), ctx)))
        self.rev = ctx.rev()  # type: int
    ...
    def updateStates(self):
        ...
        if tag:
            ...
            if exists:
                ...
                islocal = b'local' == tagtype
                try:
                    ctx = self.repo[self.repo.tags()[tag]]
                    trev = ctx.rev()  # type: int
                    thash = str(ctx)
                except:
                    trev, thash, local = 0, '????????', ''
    ...
    def set_status(self, text, icon):
        # type: (Text, Optional[Union[bool, Text, QIcon]]) -> None
    ...
    def onAddTag(self):
        ...
        if not user:
            return
        self._runTag(tagu, rev=self.rev, user=user,
