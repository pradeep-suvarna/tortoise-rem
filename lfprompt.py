...
def _createPrompt(parent, files):
    # type: (Optional[QWidget], List[bytes]) -> qtlib.CustomPrompt
...
def promptForLfiles(parent, ui, repo, files):
    # type: (Optional[QWidget], uimod.ui, localrepo.localrepository, List[bytes]) -> Optional[Tuple[List[bytes], List[bytes]]]
    lfiles = []
